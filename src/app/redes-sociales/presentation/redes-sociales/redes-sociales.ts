...
type ApiState =
  | { type: 'loading' }
  | { type: 'success'; response: RedSocialListResponse }
...
export class RedesSociales {
  ...
  private state = toSignal(
    toObservable(this.params).pipe(
      switchMap(p =>
        this.redSocialService.getAll(p).pipe(
  ...
  get redesSociales() { const s = this.state(); return s.type === 'success' ? s.response.data : []; }
  ...
  deleteRedSocial(id: number): void {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.redSocialService.delete(id).subscribe({
          next: () => {
            this.toast.success('¡Eliminada!', 'La red social ha sido eliminada');
            this.refreshTrigger.update(n => n + 1);
          },
          error: () => this.toast.error('Error', 'No se pudo eliminar la red social')
        });
      }
    });
  }
