...
type ApiState = { type: 'loading' } | { type: 'success'; response: SecretariaListResponse } | { type: 'error' } | { type: 'forbidden' };
...
export class Secretarias {
  ...
  private state = toSignal(
    toObservable(this.params).pipe(
      switchMap(p => this.secretariaService.getAll(p).pipe(
  ...
  deleteSecretaria(id: number): void {
    Swal.fire({ title: '¿Eliminar secretaría?', text: 'Esta acción no se puede deshacer', icon: 'warning',
      showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6',
      confirmButtonText: 'Sí, eliminar', cancelButtonText: 'Cancelar'
    }).then(result => {
      if (result.isConfirmed) {
        this.secretariaService.delete(id).subscribe({
          next: () => { this.toast.success('¡Eliminada!', 'La secretaría ha sido eliminada'); this.refreshTrigger.update(n => n + 1); },
          error: () => this.toast.error('Error', 'No se pudo eliminar la secretaría')
        });
      }
    });
  }
