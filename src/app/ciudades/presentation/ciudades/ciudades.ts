...
import { CiudadService } from '../../application/services/ciudad.service';
import { CiudadListResponse } from '../../domain/models/ciudad.model';
import { ToastService } from '../../../common/application/services/toast.service';
import Swal from 'sweetalert2';

type ApiState = { type: 'loading' } | { type: 'success'; response: CiudadListResponse } | { type: 'error' } | { type: 'forbidden' };
...
export class Ciudades {
  private service = inject(CiudadService);
  ...
  delete(id: number): void {
    Swal.fire({ title: '¿Eliminar ciudad?', text: 'Esta acción no se puede deshacer', icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6', confirmButtonText: 'Sí, eliminar', cancelButtonText: 'Cancelar' })
      .then(r => {
        if (r.isConfirmed) {
          this.service.delete(id).subscribe({
            next: () => { this.toast.success('¡Eliminado!', 'Ciudad eliminada'); this.refresh.update(n => n + 1); },
