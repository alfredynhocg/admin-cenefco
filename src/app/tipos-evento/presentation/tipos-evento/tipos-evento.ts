...
import { TipoEventoListResponse } from '../../domain/models/tipo-evento.model';
import { ToastService } from '../../../common/application/services/toast.service';
import Swal from 'sweetalert2';
import { extractErrorMessage } from '../../../utils/http-error';

type ApiState = { type: 'loading' } | { type: 'success'; response: TipoEventoListResponse } | { type: 'error' } | { type: 'forbidden' };
...
export class TiposEvento {
  ...
  private state = toSignal(
    toObservable(this.params).pipe(
      switchMap(p => this.tipoEventoService.getAll(p).pipe(
  ...
  get tiposEvento() { const s = this.state(); return s.type === 'success' ? s.response.data : []; }
  ...
  deleteTipoEvento(id: number): void {
    Swal.fire({ title: '¿Eliminar tipo de evento?', text: 'Esta acción no se puede deshacer', icon: 'warning',
      showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6',
      confirmButtonText: 'Sí, eliminar', cancelButtonText: 'Cancelar'
    }).then(result => {
      if (result.isConfirmed) {
        this.tipoEventoService.delete(id).subscribe({
          next: () => { this.toast.success('¡Eliminado!', 'El tipo de evento ha sido eliminado'); this.refreshTrigger.update(n => n + 1); },
          error: () => this.toast.error('Error', 'No se pudo eliminar el tipo de evento')
        });
      }
    });
  }
