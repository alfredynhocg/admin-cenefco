...
@Component({ selector: 'app-galeria-categoria-edit', imports: [ReactiveFormsModule, RouterLink, NgIcon, PageTitle], templateUrl: './galeria-categoria-edit.html' })
export class GaleriaCategoriaEdit {
  ...
  form = this.fb.group({
    nombre: ['', [Validators.required, Validators.maxLength(200)]], slug: [''],
  ...
  constructor() {
    this.service.getById(this.id).subscribe({
      ...
      error: () => { this.toast.error('Error', 'No se pudo cargar'); this.router.navigate(['/cenefco/galeria-categorias']); }
    });
  }
  ...
  onSubmit(): void {
    ...
    this.service.update(this.id, this.form.value as any).subscribe({
      next: () => { this.toast.success('¡Actualizada!', 'Categoría actualizada'); this.router.navigate(['/cenefco/galeria-categorias']); },
