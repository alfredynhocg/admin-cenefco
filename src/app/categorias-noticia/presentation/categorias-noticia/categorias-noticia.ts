...
import { CategoriaNoticiaListResponse } from '../../domain/models/categoria-noticia.model';
import { ToastService } from '../../../common/application/services/toast.service';
import Swal from 'sweetalert2';
import { extractErrorMessage } from '../../../utils/http-error';

type ApiState = { type: 'loading' } | { type: 'success'; response: CategoriaNoticiaListResponse } | { type: 'error' } | { type: 'forbidden' };
const LOADING: ApiState = { type: 'loading' };
const ERROR:   ApiState = { type: 'error' }; const FORBIDDEN: ApiState = { type: 'forbidden' };

@Component({
  selector: 'app-categorias-noticia',
  imports: [NgIcon, Pagination, PageTitle, RouterLink],
  templateUrl: './categorias-noticia.html',
  styles: ``
})
export class CategoriasNoticia {
  ...
  private state = toSignal(
    toObservable(this.params).pipe(
      switchMap(p => this.categoriaNoticiaService.getAll(p).pipe(
  ...
  get categoriasNoticia() { const s = this.state(); return s.type === 'success' ? s.response.data : []; }
  ...
  deleteCategoriaNoticia(id: number): void {
    Swal.fire({ title: '¿Eliminar categoría?', text: 'Esta acción no se puede deshacer', icon: 'warning',
      showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6',
      confirmButtonText: 'Sí, eliminar', cancelButtonText: 'Cancelar'
    }).then(result => {
      if (result.isConfirmed) {
        this.categoriaNoticiaService.delete(id).subscribe({
