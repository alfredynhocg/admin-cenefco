...
@Component({
  selector: 'app-trivia-pregunta-edit',
  imports: [ReactiveFormsModule, RouterLink, NgIcon, PageTitle],
  templateUrl: './trivia-pregunta-edit.html',
  styles: ``
})
export class TriviaPreguntaEdit implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.service.getById(this.id).subscribe({
      next: (pregunta) => {
        this.nivelService.getByCategoria(pregunta.categoria_id).subscribe({ next: (niveles) => this.niveles.set(niveles) });
        ...
        this.imgPreview.set(pregunta.imagen_url ?? null);
        this.opciones.set(pregunta.opciones.map(o => ({ texto: o.texto, es_correcta: o.es_correcta })));
      ...
      error: (err: HttpErrorResponse) => {
        this.toast.error('Error', extractErrorMessage(err, 'No se pudo cargar la pregunta'));
  ...
  onSubmit(): void {
    ...
    this.service.update(this.id, {
      ...
      next: () => {
        this.toast.success('¡Actualizada!', 'La pregunta ha sido actualizada correctamente');
        this.router.navigate(['/cenefco/trivia-preguntas']);
      },
      error: (err: HttpErrorResponse) => {
        this.toast.error('Error', extractErrorMessage(err, 'No se pudo actualizar la pregunta'));
