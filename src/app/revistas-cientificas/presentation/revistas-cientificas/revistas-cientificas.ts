...
import { RevistaCientificaListResponse } from '../../domain/models/revista-cientifica.model';
import { ToastService } from '../../../common/application/services/toast.service';
import Swal from 'sweetalert2';

type ApiState = { type: 'loading' } | { type: 'success'; response: RevistaCientificaListResponse } | { type: 'error' } | { type: 'forbidden' };
const LOADING: ApiState = { type: 'loading' };
const ERROR:   ApiState = { type: 'error' }; const FORBIDDEN: ApiState = { type: 'forbidden' };

@Component({
  selector: 'app-revistas-cientificas',
  standalone: true,
  imports: [NgIcon, Pagination, PageTitle, RouterLink, SlicePipe],
  templateUrl: './revistas-cientificas.html',
})
export class RevistasCientificas {
  ...
  delete(id: number): void {
    Swal.fire({
      title: '¿Eliminar revista científica?', text: 'Esta acción no se puede deshacer', icon: 'warning',
      showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6',
      confirmButtonText: 'Sí, eliminar', cancelButtonText: 'Cancelar',
    }).then(r => {
      if (r.isConfirmed) {
        this.service.delete(id).subscribe({
          next: () => { this.toast.success('¡Eliminado!', 'Revista científica eliminada'); this.refresh.update(n => n + 1); },
