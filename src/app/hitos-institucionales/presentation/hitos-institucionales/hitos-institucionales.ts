...
type ApiState =
  | { type: 'loading' }
  | { type: 'success'; response: HitoInstitucionalListResponse }
...
@Component({
  ...
  templateUrl: './hitos-institucionales.html',
  styles: ``
})
export class HitosInstitucionales {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.service.delete(id).subscribe({
          next: () => {
            this.toast.success('¡Eliminado!', 'Hito eliminado correctamente');
            this.refreshTrigger.update(n => n + 1);
          },
          error: () => this.toast.error('Error', 'No se pudo eliminar el hito')
        });
      }
    });
  }
