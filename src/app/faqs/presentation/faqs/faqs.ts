...
import { FaqListResponse } from '../../domain/models/faq.model';
import { ToastService } from '../../../common/application/services/toast.service';
import Swal from 'sweetalert2';

type ApiState =
  | { type: 'loading' }
  | { type: 'success'; response: FaqListResponse }
...
export class Faqs {
  ...
  get faqs()      { const s = this.state(); return s.type === 'success' ? s.response.data : []; }
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.service.delete(id).subscribe({
          next: () => {
            this.toast.success('¡Eliminada!', 'FAQ eliminada correctamente');
            this.refreshTrigger.update(n => n + 1);
          },
          error: () => this.toast.error('Error', 'No se pudo eliminar la FAQ')
        });
      }
    });
  }
