...
type ApiState = { type: 'loading' } | { type: 'success'; response: DocumentoTransparenciaListResponse } | { type: 'error' } | { type: 'forbidden' };
...
export class DocumentosTransparencia {
  ...
  private state = toSignal(
    toObservable(this.params).pipe(
      switchMap(p => this.documentoTransparenciaService.getAll(p).pipe(
  ...
  get documentosTransparencia() { const s = this.state(); return s.type === 'success' ? s.response.data : []; }
  ...
  deleteDocumentoTransparencia(id: number): void {
    Swal.fire({ title: '¿Eliminar documento?', text: 'Esta acción no se puede deshacer', icon: 'warning',
      showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6',
      confirmButtonText: 'Sí, eliminar', cancelButtonText: 'Cancelar'
    }).then(result => {
      if (result.isConfirmed) {
        this.documentoTransparenciaService.delete(id).subscribe({
          next: () => { this.toast.success('¡Eliminado!', 'El documento ha sido eliminado'); this.refreshTrigger.update(n => n + 1); },
