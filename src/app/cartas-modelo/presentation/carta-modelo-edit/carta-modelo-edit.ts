...
export class CartaModeloEdit {
  ...
  constructor() {
    this.service.getById(this.id).subscribe({
      ...
      error: () => { this.toast.error('Error', 'No se pudo cargar'); this.router.navigate(['/cenefco/cartas-modelo']); }
    });
  }
  ...
  onSubmit(): void {
    ...
    this.service.update(this.id, this.form.value as any).subscribe({
      next: () => { this.toast.success('¡Actualizado!', 'Modelo actualizado'); this.router.navigate(['/cenefco/cartas-modelo']); },
