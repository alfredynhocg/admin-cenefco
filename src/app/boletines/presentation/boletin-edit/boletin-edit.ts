...
@Component({ selector: 'app-boletin-edit', imports: [ReactiveFormsModule, RouterLink, NgIcon, PageTitle], templateUrl: './boletin-edit.html' })
export class BoletinEdit {
    ...
    constructor() {
        this.service.getById(this.id).subscribe({
            next: (d) => {
                ...
                if (d.imagen_url) this.previewUrl.set(d.imagen_url);
                this.loading.set(false);
            },
            error: () => { this.toast.error('Error', 'No se pudo cargar'); this.router.navigate(['/cenefco/boletines']); }
        });
    }
    ...
    onImageSelected(event: Event): void {
        ...
        this.service.uploadImage(file).subscribe({
            ...
            error: () => { this.toast.error('Error', 'No se pudo subir la imagen'); this.uploadingImg.set(false); }
        });
    }
    ...
    onSubmit(): void {
        ...
        this.service.update(this.id, this.form.value as any).subscribe({
            next: () => { this.toast.success('¡Actualizado!', 'Boletín actualizado'); this.router.navigate(['/cenefco/boletines']); },
