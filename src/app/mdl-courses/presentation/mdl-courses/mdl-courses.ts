...
import { MdlCourseListResponse } from '../../domain/models/mdl-course.model';
import { ToastService } from '../../../common/application/services/toast.service';
import Swal from 'sweetalert2';

type S = { type: 'loading' } | { type: 'success'; response: MdlCourseListResponse } | { type: 'error' } | { type: 'forbidden' };
const L: S = { type: 'loading' };
const E: S = { type: 'error' }; const F: S = { type: 'forbidden' };

@Component({
  selector: 'app-mdl-courses',
  imports: [NgIcon, Pagination, PageTitle, RouterLink],
  templateUrl: './mdl-courses.html',
})
export class MdlCourses {
  ...
  delete(id: number): void {
    Swal.fire({
      title: '¿Eliminar curso?', icon: 'warning',
      showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6',
      confirmButtonText: 'Sí, eliminar', cancelButtonText: 'Cancelar',
    }).then(r => {
      if (r.isConfirmed) {
        this.service.delete(id).subscribe({
          next: () => { this.toast.success('Eliminado', 'Curso eliminado'); this.refresh.update(n => n + 1); },
