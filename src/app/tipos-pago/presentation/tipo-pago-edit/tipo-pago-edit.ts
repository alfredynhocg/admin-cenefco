...
@Component({
  ...
  templateUrl: './tipo-pago-edit.html',
})
export class TipoPagoEdit {
  ...
  constructor() {
    this.service.getById(this.id).subscribe({
      ...
      error: () => { this.toast.error('Error', 'No se pudo cargar'); this.router.navigate(['/cenefco/tipos-pago']); },
    });
  }
  ...
  onSubmit(): void {
    ...
    this.service.update(this.id, this.form.value as any).subscribe({
      next: () => { this.toast.success('¡Actualizado!', 'Tipo de pago actualizado'); this.router.navigate(['/cenefco/tipos-pago']); },
