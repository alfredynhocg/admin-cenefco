...
import { EtiquetaListResponse } from '../../domain/models/etiqueta.model';
...
type ApiState =
  | { type: 'loading' }
  | { type: 'success'; response: EtiquetaListResponse }
...
export class Etiquetas {
  ...
  private state = toSignal(
    toObservable(this.params).pipe(
      switchMap(p =>
        this.etiquetaService.getAll(p).pipe(
  ...
  get etiquetas() { const s = this.state(); return s.type === 'success' ? s.response.data : []; }
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.etiquetaService.delete(id).subscribe({
          next: () => {
            this.toast.success('¡Eliminada!', 'La etiqueta ha sido eliminada');
