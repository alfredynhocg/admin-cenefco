...
import { EfectoEspecial, EfectoEspecialListResponse, TIPOS_EFECTO } from '../../domain/models/efecto-especial.model';
import { ToastService } from '../../../common/application/services/toast.service';
import Swal from 'sweetalert2';

type S = { type: 'loading' } | { type: 'success'; response: EfectoEspecialListResponse } | { type: 'error' } | { type: 'forbidden' };
const L: S = { type: 'loading' };
const E: S = { type: 'error' }; const F: S = { type: 'forbidden' };

@Component({
  selector: 'app-efectos-especiales',
  imports: [PageTitle, Pagination, RouterLink, NgIcon],
  templateUrl: './efectos-especiales.html',
})
export class EfectosEspeciales {
  ...
  tipoLabel(tipo: string): string { return TIPOS_EFECTO.find(t => t.value === tipo)?.label ?? tipo; }
  tipoEmoji(tipo: string): string { return TIPOS_EFECTO.find(t => t.value === tipo)?.emoji ?? '✨'; }

  estaActivo(item: EfectoEspecial): boolean {
    const hoy  = new Date().toISOString().slice(0, 10);
    return item.activo && item.fecha_inicio <= hoy && item.fecha_fin >= hoy;
  }
  ...
  delete(id: number): void {
    Swal.fire({
      title: '¿Eliminar efecto?', text: 'Esta acción no se puede deshacer.',
      ...
      if (r.isConfirmed) {
        this.service.delete(id).subscribe({
          next: () => { this.toast.success('Eliminado', 'Efecto eliminado'); this.refresh.update(n => n + 1); },
