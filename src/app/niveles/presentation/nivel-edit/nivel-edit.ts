...
@Component({
  ...
  templateUrl: './nivel-edit.html',
})
export class NivelEdit {
  ...
  constructor() {
    this.service.getById(this.id).subscribe({
      ...
      error: () => { this.toast.error('Error', 'No se pudo cargar'); this.router.navigate(['/cenefco/niveles']); },
    });
  }
  ...
  onSubmit(): void {
    ...
    this.service.update(this.id, this.form.value as any).subscribe({
      next: () => { this.toast.success('¡Actualizado!', 'Nivel actualizado'); this.router.navigate(['/cenefco/niveles']); },
