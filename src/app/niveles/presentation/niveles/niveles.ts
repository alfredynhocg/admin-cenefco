...
import { NivelListResponse } from '../../domain/models/nivel.model';
import { ToastService } from '../../../common/application/services/toast.service';
import Swal from 'sweetalert2';

type ApiState = { type: 'loading' } | { type: 'success'; response: NivelListResponse } | { type: 'error' } | { type: 'forbidden' };
...
export class Niveles {
  ...
  delete(id: number): void {
    Swal.fire({ title: '¿Eliminar nivel?', text: 'Esta acción no se puede deshacer', icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6', confirmButtonText: 'Sí, eliminar', cancelButtonText: 'Cancelar' })
      .then(r => {
        if (r.isConfirmed) {
          this.service.delete(id).subscribe({
            next: () => { this.toast.success('¡Eliminado!', 'Nivel eliminado'); this.refresh.update(n => n + 1); },
