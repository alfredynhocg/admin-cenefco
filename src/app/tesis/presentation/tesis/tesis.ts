...
import { TesisListResponse } from '../../domain/models/tesis.model';
import { ToastService } from '../../../common/application/services/toast.service';
import Swal from 'sweetalert2';

type ApiState = { type: 'loading' } | { type: 'success'; response: TesisListResponse } | { type: 'error' } | { type: 'forbidden' };
...
@Component({
  ...
  templateUrl: './tesis.html',
})
export class TesisList {
  ...
  delete(id: number): void {
    Swal.fire({
      title: '¿Eliminar tesis?', text: 'Esta acción no se puede deshacer', icon: 'warning',
      showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6',
      confirmButtonText: 'Sí, eliminar', cancelButtonText: 'Cancelar',
    }).then(r => {
      if (r.isConfirmed) {
        this.service.delete(id).subscribe({
          next: () => { this.toast.success('¡Eliminado!', 'Tesis eliminada'); this.refresh.update(n => n + 1); },
