...
import { ExpedidoListResponse } from '../../domain/models/expedido.model';
import { ToastService } from '../../../common/application/services/toast.service';
import Swal from 'sweetalert2';

type ApiState =
  | { type: 'loading' }
  | { type: 'success'; response: ExpedidoListResponse }
...
@Component({
  selector: 'app-expedido',
  imports: [NgIcon, Pagination, PageTitle, RouterLink],
  templateUrl: './expedido.html',
  styles: ``
})
export class Expedido {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.service.delete(id).subscribe({
          next: () => {
            this.toast.success('¡Eliminado!', 'Registro eliminado correctamente');
