...
@Component({ selector: 'app-fecha-pago-edit', imports: [ReactiveFormsModule, RouterLink, NgIcon, PageTitle], templateUrl: './fecha-pago-edit.html' })
export class FechaPagoEdit {
  ...
  form = this.fb.group({ nro_pago: [''], monto_a_pagar: [''], tipo_tramite: [''], fecha_inicio: [''], fecha_fin: [''], obligatorio: [0], estado: [1] });
  constructor() {
    this.service.getById(this.id).subscribe({
      next: (d) => { this.form.patchValue(d as any); this.loading.set(false); },
      error: () => { this.toast.error('Error', 'No se pudo cargar'); this.router.navigate(['/cenefco/fechas-pago']); }
    });
  }
  onSubmit(): void {
    this.submitting.set(true);
    this.service.update(this.id, this.form.value as any).subscribe({
      next: () => { this.toast.success('¡Actualizada!', 'Fecha actualizada'); this.router.navigate(['/cenefco/fechas-pago']); },
