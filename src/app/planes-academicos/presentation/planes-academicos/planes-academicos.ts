...
import { PlanAcademicoListResponse } from '../../domain/models/plan-academico.model';
import { ToastService } from '../../../common/application/services/toast.service';
import Swal from 'sweetalert2';

type S = { type: 'loading' } | { type: 'success'; response: PlanAcademicoListResponse } | { type: 'error' } | { type: 'forbidden' };
const L: S = { type: 'loading' }; const E: S = { type: 'error' }; const F: S = { type: 'forbidden' };

@Component({ selector: 'app-planes-academicos', imports: [NgIcon, Pagination, PageTitle, RouterLink], templateUrl: './planes-academicos.html' })
export class PlanesAcademicos {
  private service = inject(PlanAcademicoService); private toast = inject(ToastService);
