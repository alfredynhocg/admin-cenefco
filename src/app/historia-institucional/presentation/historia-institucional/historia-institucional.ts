...
import { HistoriaInstitucionalListResponse } from '../../domain/models/historia-institucional.model';
import { ToastService } from '../../../common/application/services/toast.service';
import Swal from 'sweetalert2';
import { extractErrorMessage } from '../../../utils/http-error';

type ApiState = { type: 'loading' } | { type: 'success'; response: HistoriaInstitucionalListResponse } | { type: 'error' } | { type: 'forbidden' };
const LOADING: ApiState = { type: 'loading' };
const ERROR:   ApiState = { type: 'error' }; const FORBIDDEN: ApiState = { type: 'forbidden' };

@Component({
  selector: 'app-historia-institucional',
  imports: [NgIcon, Pagination, PageTitle, RouterLink],
  templateUrl: './historia-institucional.html',
  styles: ``
})
export class HistoriaInstitucionalList {
  ...
  private state = toSignal(
    toObservable(this.params).pipe(
      switchMap(p => this.historiaInstitucionalService.getAll(p).pipe(
  ...
  get historiaInstitucional() { const s = this.state(); return s.type === 'success' ? s.response.data : []; }
  ...
  deleteHistoriaInstitucional(id: number): void {
    Swal.fire({ title: '¿Eliminar entrada?', text: 'Esta acción no se puede deshacer', icon: 'warning',
      showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6',
      confirmButtonText: 'Sí, eliminar', cancelButtonText: 'Cancelar'
    }).then(result => {
      if (result.isConfirmed) {
        this.historiaInstitucionalService.delete(id).subscribe({
          next: () => { this.toast.success('¡Eliminada!', 'La entrada ha sido eliminada'); this.refreshTrigger.update(n => n + 1); },
