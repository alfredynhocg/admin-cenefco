...
import { DirectorioInstitucionalListResponse } from '../../domain/models/directorio-institucional.model';
import { ToastService } from '../../../common/application/services/toast.service';
import Swal from 'sweetalert2';
import { extractErrorMessage } from '../../../utils/http-error';

type ApiState = { type: 'loading' } | { type: 'success'; response: DirectorioInstitucionalListResponse } | { type: 'error' } | { type: 'forbidden' };
...
@Component({
  ...
  templateUrl: './directorio-institucional.html',
  styles: ``
})
export class DirectorioInstitucionalList {
  ...
  private state = toSignal(
    toObservable(this.params).pipe(
      switchMap(p => this.directorioInstitucionalService.getAll(p).pipe(
  ...
  get directorioInstitucional() { const s = this.state(); return s.type === 'success' ? s.response.data : []; }
  ...
  deleteDirectorioInstitucional(id: number): void {
    Swal.fire({ title: '¿Eliminar entrada del directorio?', text: 'Esta acción no se puede deshacer', icon: 'warning',
      showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6',
      confirmButtonText: 'Sí, eliminar', cancelButtonText: 'Cancelar'
    }).then(result => {
      if (result.isConfirmed) {
        this.directorioInstitucionalService.delete(id).subscribe({
          next: () => { this.toast.success('¡Eliminado!', 'La entrada ha sido eliminada'); this.refreshTrigger.update(n => n + 1); },
