...
import { TipoUniversidadListResponse } from '../../domain/models/tipo-universidad.model';
import { ToastService } from '../../../common/application/services/toast.service';
import Swal from 'sweetalert2';

type ApiState = { type: 'loading' } | { type: 'success'; response: TipoUniversidadListResponse } | { type: 'error' } | { type: 'forbidden' };
const LOADING: ApiState = { type: 'loading' };
const ERROR:   ApiState = { type: 'error' }; const FORBIDDEN: ApiState = { type: 'forbidden' };

@Component({
  selector: 'app-tipos-universidad',
  standalone: true,
  imports: [NgIcon, Pagination, PageTitle, RouterLink],
  templateUrl: './tipos-universidad.html',
})
export class TiposUniversidad {
  ...
  delete(id: number): void {
    Swal.fire({ title: '¿Eliminar tipo de universidad?', text: 'Esta acción no se puede deshacer', icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6', confirmButtonText: 'Sí, eliminar', cancelButtonText: 'Cancelar' })
      .then(r => {
        if (r.isConfirmed) {
          this.service.delete(id).subscribe({
            next: () => { this.toast.success('¡Eliminado!', 'Tipo de universidad eliminado'); this.refresh.update(n => n + 1); },
