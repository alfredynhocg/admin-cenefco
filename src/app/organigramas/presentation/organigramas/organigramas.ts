...
import { OrganigramaListResponse } from '../../domain/models/organigrama.model';
import { ToastService } from '../../../common/application/services/toast.service';
import Swal from 'sweetalert2';
import { extractErrorMessage } from '../../../utils/http-error';

type ApiState = { type: 'loading' } | { type: 'success'; response: OrganigramaListResponse } | { type: 'error' } | { type: 'forbidden' };
...
export class Organigramas {
  ...
  private state = toSignal(
    toObservable(this.params).pipe(
      switchMap(p => this.organigramaService.getAll(p).pipe(
  ...
  get organigramas() { const s = this.state(); return s.type === 'success' ? s.response.data : []; }
  ...
  deleteOrganigrama(id: number): void {
    Swal.fire({ title: '¿Eliminar organigrama?', text: 'Esta acción no se puede deshacer', icon: 'warning',
      showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6',
      confirmButtonText: 'Sí, eliminar', cancelButtonText: 'Cancelar'
    }).then(result => {
      if (result.isConfirmed) {
        this.organigramaService.delete(id).subscribe({
          next: () => { this.toast.success('¡Eliminado!', 'El organigrama ha sido eliminado'); this.refreshTrigger.update(n => n + 1); },
          error: () => this.toast.error('Error', 'No se pudo eliminar el organigrama')
        });
      }
    });
  }
