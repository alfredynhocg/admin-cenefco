...
type ApiState = { type: 'loading' } | { type: 'success'; response: NoticiaListResponse } | { type: 'error' } | { type: 'forbidden' };
...
export class Noticias {
  ...
  private state = toSignal(
    toObservable(this.params).pipe(
      switchMap(p => this.noticiaService.getAll(p).pipe(
  ...
  deleteNoticia(id: number): void {
    Swal.fire({ title: '¿Eliminar noticia?', text: 'Esta acción no se puede deshacer', icon: 'warning',
      showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6',
      confirmButtonText: 'Sí, eliminar', cancelButtonText: 'Cancelar'
    }).then(result => {
      if (result.isConfirmed) {
        this.noticiaService.delete(id).subscribe({
          next: () => { this.toast.success('¡Eliminada!', 'La noticia ha sido eliminada'); this.refreshTrigger.update(n => n + 1); },
