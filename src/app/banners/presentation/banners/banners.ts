...
import { BannerListResponse } from '../../domain/models/banner.model';
...
type ApiState =
  | { type: 'loading' }
  | { type: 'success'; response: BannerListResponse }
...
export class Banners {
  ...
  private state = toSignal(
    toObservable(this.params).pipe(
      switchMap(p =>
        this.bannerService.getAll(p).pipe(
  ...
  get banners()   { const s = this.state(); return s.type === 'success' ? s.response.data : []; }
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.bannerService.delete(id).subscribe({
          next: () => {
            this.toast.success('¡Eliminado!', 'El banner ha sido eliminado');
            this.refreshTrigger.update(n => n + 1);
          },
          error: (err: HttpErrorResponse) => this.toast.error('Error', extractErrorMessage(err, 'No se pudo eliminar el banner'))
        });
      }
    });
  }
